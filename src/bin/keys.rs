    ...
    App::new("crypto")
        ...
        .subcommands(vec![
            ...
            SubCommand::with_name("inspect")
                ...
                .args_from_usage(
                    "[uri] 'A Key URI to be inspected. May be a secret seed, \
...
fn get_uri(match_name: &str, matches: &ArgMatches) -> Result<String, Error> {
    let uri = if let Some(uri) = matches.value_of(match_name) {
        ...
        if file.is_file() {
            fs::read_to_string(uri)?.trim_end().into()
        } else {
            uri.into()
        }
    } else {
...
fn execute<C: Crypto>(matches: ArgMatches) -> Result<(), Error> {
    ...
    let maybe_address_format: Option<Ss58AddressFormat> =
        match matches.value_of("format").map(|network| {
            network.try_into().map_err(|_| {
                Error::Static(
                    "Invalid address format name. See --help for available address formats.",
                )
            })
        }) {
...
fn generate_mnemonic(matches: &ArgMatches) -> Result<Mnemonic, Error> {
    let words = match matches.value_of("words") {
        Some(words) => {
            let num = usize::from_str(words)
                .map_err(|_| Error::Static("Invalid number given for --words"))?;
            MnemonicType::for_word_count(num).map_err(|_| {
                Error::Static("Invalid number of words given for phrase: must be 12/15/18/21/24")
            })?
        }
...
fn do_verify<C: Crypto>(matches: &ArgMatches, uri: &str, message: Vec<u8>) -> Result<bool, Error> {
    ...
    Ok(<<C as Crypto>::Pair as Pair>::verify(
...
fn read_pair<C: Crypto>(
    ...
    let suri = matched_suri.ok_or(Error::Static(
        "parameter is required; thus it can't be None; qed",
    ))?;
